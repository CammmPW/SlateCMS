...
export class BusinessController {
  ...
  @Post()
  async createNewBusiness(@Body() businessDTO: BusinessDTO, @GetSession() { user }: BackendUserSession): Promise<Business> {
  ...
  @Delete(':businessID')
  async deleteByID(@Param('businessID', BusinessPipe) business: BusinessEntity, @GetSession() { user }: BackendUserSession): Promise<string> {
    if (business.userID !== user.id!) {
      throw new UnauthorizedException("You cannot edit another user's business");
    }
